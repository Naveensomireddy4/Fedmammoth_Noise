...
class BaseDataset:
    ...
    def _split_fcil(
        ...
        if partition_mode in ["distribution", "extended"]:
            assert distribution_alpha is not None
        if partition_mode == "quantity":
            assert class_quantity is not None
        ...
        for split in ["train", "test"]:
            ...
            for task in range(self.N_TASKS):
                ...
                while min_samples < min_samples_split:
                    iterations += 1
                    assert iterations < 10000, "Infinite loop detected"

                    task_data = [[] for _ in range(num_clients)]
                    task_targets = [[] for _ in range(num_clients)]
                    ...
                    # Quantity-based client-class mapping
                    if partition_mode == "quantity":
                        clients_per_class = {cls: [] for cls in cur_classes}
                        for client in range(num_clients):
                            chosen = np.random.choice(
                                cur_classes, class_quantity, replace=False
                            )
                            for cls in chosen:
                                clients_per_class[cls].append(client)

                    for cls in cur_classes:
                        class_data = dataset.data[dataset.targets == cls]
                        class_targets = dataset.targets[dataset.targets == cls]
                        class_targets = class_targets % self.N_CLASSES_PER_TASK
                        ...
                        if split == "train":
                            if partition_mode in ["distribution", "extended"]:
                                ...
                                probs = np.zeros(num_clients)
                                for c in clients_per_class[cls]:
                                    probs[c] = 1 / len(clients_per_class[cls])

                            probs = np.clip(probs, 1e-12, 1)
                            assigned_client = np.random.choice(
                                num_clients, size=num_samples, p=probs
                            )

                            num_samples_per_client_task.append(
                                [(assigned_client == i).sum() for i in range(num_clients)]
                            )
                        else:
                            assigned_client = np.random.choice(
                                num_clients, size=num_samples
                            )

                        for client in range(num_clients):
                            task_data[client].append(
                                class_data[assigned_client == client]
                            )
                            task_targets[client].append(
                                class_targets[assigned_client == client]
                            )

                    # Concatenate per client
                    task_data = [
                        np.concatenate(client_data)
                        if len(client_data) > 0
                        else np.empty((0,))
                        for client_data in task_data
                    ]
                    task_targets = [
                        np.concatenate(client_targets)
                        if len(client_targets) > 0
                        else np.empty((0,))
                        for client_targets in task_targets
                    ]

                    min_samples = min(len(td) for td in task_data)

                # Store per-task statistics
                for cls_idx in range(len(num_samples_per_client_task)):
                    num_samples_per_client.setdefault(cls_idx, []).append(
                        num_samples_per_client_task[cls_idx]
                    )
                ...
                # 🔍 DEBUG PRINTS (THIS IS WHAT YOU ASKED FOR)
                print(f"[{split.upper()}] Task {task} summary:")
                for c in range(num_clients):
                    print(
                        f"  Client {c}: {len(task_data[c])} samples"
                    )
                print(
                    f"  → Total samples: {sum(len(td) for td in task_data)}"
                )

        # Final summary
        for split in ["train", "test"]:
            total = sum(
                len(getattr(self, f"{split}_data")[t][c])
                for t in range(len(getattr(self, f"{split}_data")))
                for c in range(num_clients)
            )
            print(f"\n[{split.upper()}] TOTAL samples across all tasks & clients: {total}")
        ...
        for split in ["train", "test"]:
            ...
            split_targets = getattr(self, f"{split}_targets")

            for task in range(len(split_targets)):
                print(f"\nTask {task}:")

                for client in range(num_clients):
                    labels = split_targets[task][client]

                    if len(labels) == 0:
                        print(f"  Client {client}: EMPTY")
                        continue
                    ...
                    print(f"  Client {client} → [{label_info}]")
