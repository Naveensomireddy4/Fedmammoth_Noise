...
in_channels = {
    "mnist": 1, "KMNIST": 1, "EMNIST": 1, "FashionMNIST": 1, "SVHN": 3,
    "seq-cifar10": 3, "seq-cifar100": 3, "CINIC10": 3, "seq-tinyimagenet": 3,
    "imagenet100": 3, "imagenet1000": 3,
    "bloodmnist": 3, "tissuemnist": 1, "pathmnist": 3,
    "organamnist": 1, "organcmnist": 1, "organsmnist": 1,
    "newsgroup": None, "reuters": None, "yahoo": None, "dbpedia": None,
}
inp_size = {
...
def get_mean(args):
    mean = {
        ...
        "seq-cifar10": (0.4914, 0.4822, 0.4465),
        "seq-cifar100": (0.5071, 0.4867, 0.4408),
        "CINIC10": (0.47889522, 0.47227842, 0.43047404),
        "seq-tinyimagenet": (0.4802, 0.4481, 0.3975), # (0.4914, 0.4822, 0.4465) https://github.com/AlbinSou/ocl_survey/blob/main/src/factories/benchmark_factory.py
...
class Memory:
    def __init__(self, args):
        ...
        self.args = args
        args.input_size = (in_channels[args.dataset], inp_size[args.dataset], inp_size[args.dataset])
        if args.dataset in ['newsgroup', 'reuters', 'yahoo', 'dbpedia']:
    ...
    def random_sampling(self, subsample_size, exclude_task=None):
        if exclude_task is not None:
            ...
            # if only one element, make it 1D
            valid_indices = valid_indices.view(-1)

            actual_size = min(subsample_size, len(valid_indices))
            ...
            available_size = min(self.seen, self.x.size(0))

            if available_size <= subsample_size:
                return self.x[:available_size], self.y[:available_size], self.t[:available_size]

            indices = torch.from_numpy(
                np.random.choice(available_size, subsample_size, replace=False)
            )
    ...
    def uncertainty_sampling(self, model, mem_x=None, mem_y=None, mem_t=None,
        ...
        unc_scores, descending_flag = compute_uncertainty_scores(self.args, mem_x,mem_t, model, self.augmentations, seen_cls=self.seen_classes)
...
def compute_uncertainty_scores(args, mem_x,mem_t, model, augmentations, tta_rep=5, seen_cls=None):
    ...
    if args.dataset in ['newsgroup', 'reuters', 'yahoo', 'dbpedia']:
        ...
        all_logits = []
        with torch.no_grad():
            task_id = int(mem_t[0].item())   # take scalar task id

            for rep in range(tta_rep):
                bx_tmp = add_gaussian_noise(args, mem_x)
                logits_tmp = model(bx_tmp, task_id=task_id)
        ...
        all_logits = []
        with torch.no_grad():
            task_id = int(mem_t[0].item())   # scalar task id
